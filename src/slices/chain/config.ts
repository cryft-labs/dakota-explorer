...
import type { NetworkVerificationType, NetworkVerificationTypeEnvs } from 'src/slices/chain/verification-type/types/config';

import { getEnvValue, getExternalAssetFilePath, parseEnvJson } from 'src/config/utils/envs';

import { urlValidator } from 'src/toolkit/components/forms/validators/url';

const DEFAULT_CURRENCY_DECIMALS = 18;
const DAKOTA_BRAND_ASSET = '/assets/dakota/dakota-rabbit-logo.svg';
...
const chain = Object.freeze({
  ...
  name: getEnvValue('NEXT_PUBLIC_NETWORK_NAME') || 'Dakota Explorer',
  shortName: getEnvValue('NEXT_PUBLIC_NETWORK_SHORT_NAME') || 'Dakota',
  currency: {
  ...
  logo: {
    'default': getExternalAssetFilePath('NEXT_PUBLIC_NETWORK_LOGO') || DAKOTA_BRAND_ASSET,
    dark: getExternalAssetFilePath('NEXT_PUBLIC_NETWORK_LOGO_DARK') || DAKOTA_BRAND_ASSET,
  },
  icon: {
    'default': getExternalAssetFilePath('NEXT_PUBLIC_NETWORK_ICON') || DAKOTA_BRAND_ASSET,
    dark: getExternalAssetFilePath('NEXT_PUBLIC_NETWORK_ICON_DARK') || DAKOTA_BRAND_ASSET,
  },
  rpcUrls,
