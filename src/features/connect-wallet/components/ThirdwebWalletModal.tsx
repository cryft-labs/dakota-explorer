...
import { EIP1193, getWalletInfo, injectedProvider, walletConnect } from 'thirdweb/wallets';
...
interface WalletConnectPromptState {
  label: string;
  uri?: string;
}

interface WalletConnectPromptProps extends WalletConnectPromptState {
  onCancel: () => void;
}

const WalletConnectPrompt = ({ label, uri, onCancel }: WalletConnectPromptProps) => {
  ...
  React.useEffect(() => {
    ...
    QRCode.toString(uri, { margin: 1, type: 'svg', width: 240 }, (error, svg) => {
      if (!isActive) {
        return;
      }

      if (error) {
        setQrError('The secure pairing code could not be generated. Please return and try again.');
        return;
      }
  ...
  const handleCopy = React.useCallback(async() => {
    ...
    try {
      ...
      setQrError('The pairing link could not be copied. Please scan the code instead.');
    }
  }, [ uri ]);

  const title = label === 'WalletConnect' ? 'Connect with WalletConnect' : `Connect ${ label }`;
  const instruction = label === 'WalletConnect' ?
    'Scan this code with any WalletConnect-compatible wallet.' :
    `Scan this code from ${ label }, or use the installed ${ label } browser extension.`;

  return (
    <Stack gap={ 4 } alignItems="center" textAlign="center">
      <Box>
        <Text fontWeight={ 700 }>{ title }</Text>
        <Text color="text.secondary" fontSize="sm" lineHeight="tall" mt={ 1 }>
          { instruction } Keep this window open while you approve the connection.
        </Text>
      </Box>
      ...
      >
        { qr ? (
          <Box w="full" h="full" dangerouslySetInnerHTML={{ __html: qr }}/>
        ) : (
          <Text role="status" color="#315b53" fontSize="sm" px={ 4 }>
            { qrError || 'Preparing secure pairing code...' }
          </Text>
        ) }
      </Box>

      { qrError && qr && (
        <Text role="alert" color={{ _light: 'red.700', _dark: 'red.200' }} fontSize="sm">
          { qrError }
        </Text>
      ) }

      <Text color="text.secondary" fontSize="xs">
        On mobile, open the explorer inside your wallet browser or scan this code from another device.
      </Text>
...
const ThirdwebWalletModal = ({ view, onClose, onConnected }: ModalProps) => {
  ...
  const [ walletConnectPrompt, setWalletConnectPrompt ] = React.useState<WalletConnectPromptState>();
  ...
  const handleClose = React.useCallback(() => {
    ...
    setWalletConnectPrompt(undefined);
    setError(undefined);
    setDragOffset(0);
    setIsDragging(false);
    dragOffsetRef.current = 0;
    onClose();
  }, [ cancelConnection, onClose, view ]);
  ...
  const runConnection = React.useCallback(async(action: string, createConnection: (attemptId: number) => Promise<Wallet>) => {
    ...
    try {
      const wallet = await createConnection(attemptId);
      ...
      if (!connectedWallet) {
        throw new Error('The wallet connected but could not be activated in the explorer.');
      }

      if (attemptId !== connectionAttemptRef.current) {
        disconnectWallet(connectedWallet);
        return;
      }
  ...
  }, [ cancelConnection, connect, disconnectWallet, onConnected ]);
  ...
  const handleCancelWalletConnect = React.useCallback(() => {
    connectionAttemptRef.current += 1;
    cancelConnection();
    setPendingAction(undefined);
    setWalletConnectPrompt(undefined);
  ...
  const handleExternalConnect = React.useCallback((walletId: ExternalWalletId) => {
    const label = EXTERNAL_WALLETS.find((wallet) => wallet.id === walletId)?.label ?? 'wallet';

    void runConnection(walletId, async(attemptId) => {
      if (walletId !== 'walletConnect') {
        const provider = injectedProvider(walletId);

        if (provider) {
          const wallet = EIP1193.fromProvider({ provider, walletId });
      ...
      await wallet.connect({
        ...
        onDisplayUri: (uri) => {
          if (attemptId === connectionAttemptRef.current) {
            setWalletConnectPrompt({ label, uri });
          }
        },
        optionalChains: thirdwebChains,
        showQrModal: false,
      });
  ...
  const handleDisconnect = React.useCallback(() => {
    ...
    onClose();
  }, [ activeWallet, disconnectWallet, onClose ]);
  ...
  const connectContent = walletConnectPrompt ? (
    <WalletConnectPrompt
      label={ walletConnectPrompt.label }
      uri={ walletConnectPrompt.uri }
